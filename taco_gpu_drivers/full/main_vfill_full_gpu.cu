...
__global__
void computeDeviceKernel0(taco_tensor_t * __restrict__ a, float &b_val){
  int a1_dimension = (int)(a->dimensions[0]);
  float* __restrict__ a_vals = (float*)(a->vals);

  int32_t i31 = blockIdx.x;
  int32_t i32 = (threadIdx.x % (256));
  if (threadIdx.x >= 256) {
    return;
  }

  int32_t i = i31 * 256 + i32;
  if (i >= a1_dimension)
    return;
...
int compute(taco_tensor_t *a, taco_tensor_t *b) {
  ...
  computeDeviceKernel0<<<((a1_dimension + 255) / 256), 256>>>(a, b_val);
