...
int main(int argc, char* argv[]){
  ...
  int N = 1000;
  float* h = (float*)malloc(1000 * 1000 * sizeof(float));
  float* x = (float*)malloc(1000 * sizeof(float));
  float* y = (float*)malloc(1000 * sizeof(float));

  int h_dims[2] = {1000,1000};
  taco_tensor_t* h_tt = init_taco_tensor(2, sizeof(float), h_dims);
  h_tt->vals = h;
  h_tt = init_taco_tensor_gpu(h_tt);
  int x_dims[1] = {1000};
