...
__global__
void computeDeviceKernel0(taco_tensor_t * __restrict__ a, taco_tensor_t * __restrict__ b, float &c_val){
  float* __restrict__ a_vals = (float*)(a->vals);
  int b1_dimension = (int)(b->dimensions[0]);
  float* __restrict__ b_vals = (float*)(b->vals);

  int32_t i45 = blockIdx.x;
  int32_t i46 = (threadIdx.x % (256));
  if (threadIdx.x >= 256) {
    return;
  }

  int32_t i = i45 * 256 + i46;
  if (i >= b1_dimension)
    return;
...
int compute(taco_tensor_t *a, taco_tensor_t *b, taco_tensor_t *c) {
  ...
  computeDeviceKernel0<<<((b1_dimension + 255) / 256), 256>>>(a, b, c_val);
...
int main(int argc, char* argv[]){
  ...
  int v_dims[1] = {1};
  taco_tensor_t* v_tt = init_taco_tensor(1, sizeof(float), v_dims);
  v_tt->vals = &v;
  v_tt = init_taco_tensor_gpu(v_tt);
